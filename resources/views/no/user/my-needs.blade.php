		...
		@foreach ($userNeeds as $need)
			<div class="standardBox" style="padding: 5px;">
				...
				@if($need->gyldig == false)
					<p><b style="color: red">Ikke lenger gyldig</b></p>
				@else
					<p><b style="color: green">Gyldig</b></p>
					<button id="{{ $IDCount }}" type="button" value="{{ $need->id }}" class="btn btn-info moreInfoButtonUser">Mer info</button>
					<a href= '/no/bruker/slett-hjelp/{{ $need->id }}'>Slett</a>
				@endif
