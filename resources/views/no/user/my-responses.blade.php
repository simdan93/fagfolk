@extends('no.layouts.user_dashboard')
@section('title', 'Responser fra fagfolk')
@section('page_heading','Responser fra fagfolk')

@section('dashboard-content')
	@if(session()->has('mailNotSent'))
		<div class="alert alert-failure">
			{{ session()->get('mailNotSent') }}
		</div>
	@elseif(session()->has('mailSent'))
		<div class="alert alert-succsess">
			{{ session()->get('mailSent') }}
		</div>
	@endif
	@if($responseInfo == null)
		<p>Ingen respons ennå</p>
	@else
		@php ($surveyCount = 0)
		@php ($visitedCount = 0)
		@php ($tmpCount1 = 0)

		@foreach ($responseInfo as $response)
			<!--
			FOR EACH RESPONSE,
			1. CHECK IF IT IS A SURVEY RESPONSE.
			2. CREATE A BOX
			3. Go through the responses again beginning from the unvisited responses and check if it has the same value as prev. respons.
			-->
			@if ($tmpCount1 < $visitedCount)
				@php ($tmpCount1 += 1)
				@continue
			@endif
			@if($response->befaring == true)
				@php ($surveyCount += 1)
				@if($response->antall_aksepterte_befaringer == 5)
					<div class="surveyBox inactive" >
				@else
					<div class="surveyBox" >
				@endif
					<p><b>Befaring - {{ $response->antall_aksepterte_befaringer }}/5</b></p>
					@php ($tmpCount2 = 0)
					@foreach ($responseInfo as $response2)
						@if ($tmpCount2 < $visitedCount)
							@php ($tmpCount2 += 1)
							@continue
						@endif
						@if ($response->userneed_id != $response2->userneed_id)
							@break
						@else
							@php ($visitedCount += 1)
							@if($response->antall_aksepterte_befaringer == 5)
								<div class="responseFromWorker surveyBox infoBox inactive">
							@else
								<div class="responseFromWorker surveyBox infoBox">
							@endif
								@if($response2->ignorert == false)
									<p>
										<b>Selskap:</b> {{ $response2->selskap }}
										<b>Org. nr </b> {{ $response2->org_nummer }}
									</p>
									<div class="workarea">
										<p><b>Prosjektområde: </b>{{ $response2->postnummer }}</p>
									</div>
									<p>
										<b>Hovedfag: </b> {{ $response2->mainservice_id }}  <b>Spesialisering: </b> {{ $response2->secondaryservice_id }}
									</p>
									<p>
										<b>Timepris:</b> {{ $response2->timepris }}   <b>Oppmøtepris:</b> {{ $response2->oppmøtepris }}
									</p>
									<p><b>Status: </b>
									@if($response->akseptert == true)
										Akseptert</p>
									@else
										Venter respons</p>
									@endif
									@if($response2->akseptert != true)
										<p>
											<a href= '/no/bruker/aksepter-respons/{{ $response2->id }}/{{ $response2->userneed_id }}'>Aksepter | </a>
											<a href= '/no/bruker/ignorer-respons/{{ $response2->id}}/{{ $response2->userneed_id }}'>Ignorer</a>
										</p>
									@else
									@endif
								<button id="{{ $visitedCount }}" type="button" value="{{ $response2->response_message }}" class="btn btn-warning msgFromWorker">Se melding fra selskap</button>
								@endif
								<div id="msgFromWorker{{ $visitedCount }}" style="display:none; padding: 5px;">
									<p id="tilgjengelig"></p>
									<p id="svarFraFagman"></p>
								</div>
							</div>
						@endif
						@php ($tmpCount2 += 1)
					@endforeach
				</div>
			@else
					@php ($visitedCount += 1)
					@if($response->gyldig == false)
						<div class="singleBox inactive" >
							<p><b>Vanlig - Ugyldig</b></p>
					@else
						<div class="singleBox" >
							<p><b>Vanlig - <span style="color: green">Gyldig</span></b></p>
					@endif
					@if($response->gyldig == false)
						<div class="responseFromWorker singleBox infoBox inactive">
					@else
						<div class="responseFromWorker singleBox infoBox">
					@endif
							@if($response->ignorert == false)
								<p>
									<b>Selskap:</b> {{ $response->selskap }}
									<b>Org. nr </b> {{ $response->org_nummer }}
								</p>
								<div class="workarea">
									<p><b>Prosjektområde: </b>{{ $response->postnummer }}</p>
								</div>
								<p>
									<b>Hovedfag: </b> {{ $response->mainservice_id }}  <b>Spesialisering: </b> {{ $response->secondaryservice_id }}
								</p>
								<p>
									<b>Timepris:</b> {{ $response->timepris }}   <b>Oppmøtepris:</b> {{ $response->oppmøtepris }}
								</p>
								<p><b>Status: </b>
								@if($response->akseptert == true)
									Akseptert</p>
								@else
									Venter respons</p>
								@endif
								@if($response->gyldig != false)
									<p>
										<a href= '/no/bruker/aksepter-respons/{{ $response->id }}/{{ $response->userneed_id }}'>Aksepter | </a>
										<a href= '/no/bruker/ignorer-respons/{{ $response->id}}/{{ $response->userneed_id }}'>Ignorer</a>
									</p>
								@endif
								<button id="{{ $visitedCount }}" type="button" value="{{ $response->response_message }}" class="btn btn-warning msgFromWorker">Se melding fra selskap</button>
							@endif
							<div id="msgFromWorker{{ $visitedCount }}" style="display:none; padding: 5px;">
								<p id="tilgjengelig"></p>
								<p id="svarFraFagman"></p>
							</div>
						</div>
					</div>
				@endif
				@php ($tmpCount1 += 1)
		@endforeach
	@endif
@stop

@section('right-sidebar-content')
@stop

@section('scripts')
<script src="/js/getResponseMsg.js"></script>
@stop
